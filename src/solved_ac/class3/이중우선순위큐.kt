 ...
 *
 * @see <a href="https://www.acmicpc.net/problem/7662">이중 우선순위 큐</a>
 * */
fun main() {
    val question = readln()
    ...
    for (c in 0 until question.toInt()) {
        treeMap.clear()

        for (i in 0 until readln().toInt()) {
            val (op, valueString) = readln().split(" ")
            val value = valueString.toInt()
            when (op) {
                "I" -> {
                    val pre = treeMap.getOrPut(value) { 0 }
                    treeMap[value] = pre + 1
                }
                ...
                "D" -> {
                    ...
                    if (value == 1) {
                        val (highest, count) = treeMap.lastEntry()
                        if (count == 1) treeMap.pollLastEntry() else treeMap[highest] = count - 1
                    } else {
                        val (lowest, count) = treeMap.firstEntry()
                        if (count == 1) treeMap.pollFirstEntry() else treeMap[lowest] = count - 1
                    }
                }
            }
        }

        val result = if (treeMap.isEmpty()) "EMPTY" else "${treeMap.lastKey()} ${treeMap.firstKey()}"
