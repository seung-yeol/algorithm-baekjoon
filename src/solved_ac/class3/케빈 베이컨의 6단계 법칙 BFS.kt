...
fun main() {
    ...
    val connections = Array(count + 1) { mutableSetOf<Int>() }
    ...
    val solver = Q1389(connections)
    println(solver.answer())
}

private class Q1389(val connections: Array<MutableSet<Int>>) {
    private var answer = 0
    private var min = Int.MAX_VALUE

    fun answer(): Int {
        for (index in 1..connections.lastIndex) {
            val visited = BooleanArray(connections.size + 1) { false }.apply { set(index, true) }
            val newKevinBacon = bfs(index = index, step = 1, visited)

            if (min <= newKevinBacon) continue
            min = newKevinBacon
            answer = index
        }
    ...
    private fun bfs(index: Int, step: Int, visited: BooleanArray): Int {
        ...
        for (i in queue) {
            connectedCount += bfs(i, step + 1, visited)
        }
