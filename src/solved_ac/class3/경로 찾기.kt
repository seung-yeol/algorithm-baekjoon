 ...
 *
 * @see <a href="https://www.acmicpc.net/problem/11403">경로 찾기</a>
 * */
fun main() {
    val n = readln().toInt()
    ...
    repeat(n) { y ->
        for ((index, c) in readln().split(" ").map(String::toInt).withIndex()) {
            matrix[y][index] = c
        }
    }
...
class Q11403 {
    fun solve(matrix: Array<IntArray>) {
        for (stopover in matrix.indices) {
            for ((start, ints) in matrix.withIndex()) {
                for ((destination, value) in ints.withIndex()) {
                    if (value == 1) continue

                    if (matrix[start][stopover] == 1 && matrix[stopover][destination] == 1) {
