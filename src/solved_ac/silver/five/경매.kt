 ...
 *
 * @see <a href="https://www.acmicpc.net/problem/2238">경매</a>
 * */
fun main() {
    val (limit, size) = readln().split(" ").map(String::toInt)

    val bidCounts = HashMap<Int, Int>()
    val people = Array(limit + 1) { "" }
    for (i in 0 until size) {
        val (name, price) = readln().split(" ").run { first() to last().toInt() }

        bidCounts[price] = bidCounts.getOrDefault(price, 0) + 1
        if (people[price] == "") people[price] = name
    }

    val (price, _) = PriorityQueue { t: Map.Entry<Int, Int>, t2: Map.Entry<Int, Int> -> t.value.compareTo(t2.value) }
        .run {
            addAll(bidCounts.entries)
            poll()
        }

    println("${people[price]} $price")
}
