 ...
 *
 * @see <a href="https://www.acmicpc.net/problem/1544">사이클 단어</a>
 * */
fun main() {
    ...
    repeat(count) {
        val input = readln()
        val words = map.getOrPut(input.length) { mutableListOf() }

        var isEqual = false
        task@ for (word in words) {
            subtask@ for (i in word.indices) {
                for ((index, char) in input.withIndex()) {
                    if (word[(index + i) % word.length] != char) continue@subtask
                }
        ...
        if (!isEqual) words.add(input)
    }

    var result = 0
    for ((_, words) in map) result += words.size
